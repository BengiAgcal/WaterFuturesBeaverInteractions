...
namespace KWS
{
    public class KWS_EditorSplineMesh
    {
        ...
        Vector3 DrawMoveHandle(Vector3 position, Color color)
        {
            var newPos = position;
            Handles.color = color;
            var fmh_189_53_638225204144898975 = Quaternion.identity; newPos = Handles.FreeMoveHandle(newPos, GetHandleSphereSize(position), Vector3.one, Handles.CylinderHandleCap);
            return newPos;
        }
